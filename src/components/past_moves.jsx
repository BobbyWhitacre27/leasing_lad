...
const Past_moves = ({ user }) => {
    ...
    const residentCard = sortByDate.map((c) => {
        ...
        const approvalDocs = JSON.stringify(c.sent_approval_docs);
        const leaseSent = JSON.stringify(c.sent_lease);
        const electricSetUp = JSON.stringify(c.received_electric);
        const insuranceSetUp = JSON.stringify(c.received_insurance);
        const leaseSigned = JSON.stringify(c.received_signed_lease);
        const paymentMade = JSON.stringify(c.received_payment);
        ...
        // function calclulatePercentComplete(approvalDocsSent, lease_Sent, electric_SetUp, insurance_SetUp, lease_Signed, payment_Made) {
        //     const total = 0
        //     if(approvalDocsSent == "true"){
        //         total += 1
        //     }if(lease_Sent == "true"){
        //         total += 1
        //     }if(electric_SetUp == "true"){
        //         total += 1
        //     }if(insurance_SetUp == "true"){
        //         total +=1
        //     }if(lease_Signed == "true"){
        //         total +=1
